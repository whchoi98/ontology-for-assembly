...
def get_object(
    ...
    # Person 상세 카드 enrich (objects_catalog 경로 _list_persons → fetch_members 보강):
    #  1) profile_image_url=None → member_directory 국회 공식 사진 URL로 채움
    #     (ADR-0004 "회색 placeholder 금지" — 사용자 신고 2026-06-04 "초"/"조" 텍스트).
    #  2) analytics 9 메트릭 전체를 member_directory(MemberAnalytics SSOT)로 채움.
    #     objects_catalog analytics는 bills_co_proposed·floor_votes·party_alignment_pct·
    #     statements를 생성하지 않아 detail-card 칩이 비던 문제 (사용자 신고 2026-06-16, 1WE5693J 김태년).
    if class_name == "Person":
        from api.services import member_directory as _md
        _m = _md.get_member(_md.resolve_id(instance_id))
        if _m is not None:
            if not matched.get("profile_image_url") and _m.profile_image_url:
                matched["profile_image_url"] = _m.profile_image_url
            _a = _m.analytics
            matched["analytics"] = {
                "plenary_attendance_pct": _a.plenary_attendance_pct,
                "committee_attendance_pct": _a.committee_attendance_pct,
                "bills_proposed": _a.bills_proposed,
                "bills_co_proposed": _a.bills_co_proposed,
                "floor_votes": _a.floor_votes,
                "party_alignment_pct": _a.party_alignment_pct,
                "statements": _a.statements,
                "media_mentions_30d": _a.media_mentions_30d,
                "composite_score": _a.composite_score,
            }
